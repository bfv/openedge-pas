rm -rf /usr/dlc/*OpenEdge*
rm -rf /usr/dlc/*sports*
rm -f  /usr/dlc/ade.pf
rm -rf /usr/dlc/ant/manual
rm -f  /usr/dlc/demo*
rm -rf /usr/dlc/dotnet
rm -f  /usr/dlc/empty*
rm -rf /usr/dlc/esbadapter
rm -rf /usr/dlc/gradle
rm -f  /usr/dlc/inst_hlp
#rm -rf /usr/dlc/install*  ## DO NOT DELETE FOR 12.8
rm -f /usr/dlc/install/esam/*

rm -rf /usr/dlc/java/ext
rm -f  /usr/dlc/newk*
rm -rf /usr/dlc/odbc
rm -rf /usr/dlc/oebuild
rm -rf /usr/dlc/ora
#rm -rf /usr/dlc/perl
rm -rf /usr/dlc/prohelp
rm -rf /usr/dlc/prolang/ame
rm -rf /usr/dlc/prolang/utf
rm -rf /usr/dlc/servers/redist
rm -rf /usr/dlc/sonic
rm -rf /usr/dlc/src
rm -rf /usr/dlc/templates
rm -rf /usr/dlc/toolkit
rm -rf /usr/dlc/tty/adm
rm -rf /usr/dlc/tty/adm2
rm -rf /usr/dlc/tty/prodict
rm -f /usr/dlc/tty/prodict.*
rm -rf /usr/dlc/ubqmanager

# /usr/dlc/bin
rm -f /usr/dlc/bin/*sql*
#rm -f /usr/dlc/bin/*sts*
rm -f /usr/dlc/bin/_bprowsdldoc
rm -f /usr/dlc/bin/_mprosrv
rm -f /usr/dlc/bin/_mproshut
rm -f /usr/dlc/bin/dbtool
rm -f /usr/dlc/bin/xsdto4gl

# bin
rm -rf /usr/dlc/bin/_dbagent
rm -rf /usr/dlc/bin/_dbutil
rm -rf /usr/dlc/bin/_mprshut
rm -rf /usr/dlc/bin/_mtprosrv
rm -rf /usr/dlc/bin/_pphpriv
rm -rf /usr/dlc/bin/_progres
rm -rf /usr/dlc/bin/_proutil
rm -rf /usr/dlc/bin/_rfutil 
rm -rf /usr/dlc/bin/_rocket.dll

# oe_oemgmt
rm -rf /usr/oemgmt
rm -rf /usr/wrk_oemgmt

# other
rm -rf /var/cache
